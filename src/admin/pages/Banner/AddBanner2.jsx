...
function AddBanner2() {
    ...
    const addBanner = async(e) =>{
        e.preventDefault()
        try {
            const response = await axios.post(`http://localhost:3002/api/banner2`,{image:image,title:title,description:description,percentage:percentage,rate:rate,offerRate:offerRate},{headers:{
  ...
  return (
    <>
    <div className=" border-2 border-teal-800 shadow p-10 rounded">
        <form className='p-3' action="" onSubmit={addBanner}>
            <p className='text-base sm:text-lg mt-3 p-3 text-center'>Add Banner 2</p>
