...
function Usercart() {
  ...
  const handleIncrementQuantity = async (id) => {
    try {
      const response = await axios.post(`http://localhost:3002/api/cart/increment-cart/${JSON.parse(localStorage.getItem("userData"))._id}/${id}`)
      setRefresh(!refresh)
    } catch (error) {
      console.log(error);
    }
  ...
  const handleDecrementQuantity = async (id) => {
    try {
      const response = await axios.get(`http://localhost:3002/api/cart/decrement-cart/${JSON.parse(localStorage.getItem("userData"))._id}/${id}`)
      setRefresh(!refresh)
    } catch (error) {
      console.log(error);
    }
  ...
  const handleRemoveQuantity = async (id) => {
    try {
      const response = await axios.get(`http://localhost:3002/api/cart/remove-cart/${JSON.parse(localStorage.getItem("userData"))._id}/${id}`)
      setRefresh(!refresh)
          ...
          <div className="flex flex-wrap justify-center">
            ...
            <div className="bg-slate-100 m-4 border shadow p-5 rounded-lg lg:w-[700px] md:w-[550px]">
              ...
              <div className="m-3 flex flex-wrap">
                {
                  cart.map((item) => {
                    console.log(totalAmount, typeof totalAmount,'total');
              ...
              <div className="border-1 p-2 shadow right-4">
                ...
                <div className="text-center justify-center items-center flex">
                  <input className="w-44 rounded border m-2 p-2" type="text" placeholder="Have a coupon?" /> <span onClick={() => {
                    // (totalAmount - code) = totalAmount
                  }} className="absolute ms-[7.75rem] fw-semibold hover:text-pink-800 text-teal-800">Apply</span>
                </div>
